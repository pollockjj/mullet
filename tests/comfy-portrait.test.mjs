...
test('exposes only the compatible model assets and zimage LoRAs', async () => {
  const replies = [
    ...
    { CLIPLoader: { input: { required: {
      clip_name: [['qwen_3_4b.safetensors', 'qwen_2.5_vl_7b_fp8_scaled.safetensors']],
      type: [['lumina2', 'qwen_image']]
    } } } },
    nodeInfo('VAELoader', 'vae_name', ['ae.safetensors', 'qwen_image_vae.safetensors']),
    nodeInfo('LoraLoader', 'lora_name', [
      'flux/other.safetensors',
      'zimage/kristi6.safetensors',
      'Qwen-Image-Edit-2511-Lightning-4steps-V1.0-bf16.safetensors'
    ]),
    ...QWEN_IMAGE_EDIT_REFERENCE_TEMPLATE.requiredNodes.map(nodePresent)
  ];
  let index = 0;
  ...
  assert.deepEqual(capabilities.aspectRatios, [{ id: '2:3', width: 2, height: 3, label: '2:3 fixed expression' }]);
  assert.equal(capabilities.megapixels[0], 0.5);
  assert.equal(capabilities.referenceTemplate.id, PORTRAIT_REFERENCE_TEMPLATE_ID);
});

test('verifies the Jenna reference bytes before queuing the Qwen Image Edit identity graph', async () => {
  ...
  assert.equal(queued.prompt['5'].class_type, 'FluxKontextImageScale');
  assert.equal(queued.prompt['7'].class_type, 'CFGNorm');
  assert.equal(queued.prompt['8'].class_type, 'LoraLoaderModelOnly');
  assert.equal(queued.prompt['9'].class_type, 'TextEncodeQwenImageEditPlus');
  assert.equal(queued.prompt['12'].inputs.seed, 19790213);
  assert.deepEqual(queued.prompt['15'].inputs, {
...
test('queues, polls, and proxies only the fixed portrait output', async () => {
  ...
  assert.deepEqual(queued.prompt['7'].inputs, { width: 768, height: 1152, batch_size: 1 });
...
test('rejects a Comfy portrait whose PNG IHDR is not the requested fixed 2:3 frame', async () => {
  const fetcher = async (input) => {
    ...
    if (url.includes('/view?')) return new Response(png(768, 1024), { headers: { 'content-type': 'image/png' } });
    throw new Error(`unexpected URL ${url}`);
  };
