...
const mp4FiveBytes = buildH264AacMp4Fixture({ width: 768, height: 768, frames: 124, includeAudio: false });
const audioMp4Bytes = buildH264AacMp4Fixture({ width: 768, height: 768, frames: 73 });
...
function motionRequest(imageSha256, mode = 'i2v', durationSeconds = 3) {
  return {
    ...
    endFrameModelTemplate: mode === 'flf2v_generated' ? 'mage-flow-edit-turbo-4step-v1' : null,
    mode,
    source: {
...
test('compiled portrait-video route enforces the fake-Comfy contract', { timeout: 120_000 }, async (context) => {
  ...
  await context.test('GET returns exact MiniMax H3 capabilities through the compiled route', async () => {
    ...
    assert.deepEqual(
      ...
      new Set([...MINIMAX_H3_PORTRAIT_VIDEO_TEMPLATE.requiredNodes, ...MAGE_FLOW_EDIT_PORTRAIT_END_FRAME_TEMPLATE.requiredNodes])
    );
    assert.equal(
      queriedNodes.length,
      MINIMAX_H3_PORTRAIT_VIDEO_TEMPLATE.requiredNodes.length + MAGE_FLOW_EDIT_PORTRAIT_END_FRAME_TEMPLATE.requiredNodes.length
    );
  });
  ...
  await context.test('POST generates a Mage-Flow end frame before queuing distinct-frame FLF motion', async () => {
    fake.reset();
