...
const mp4FiveBytes = buildH264AacMp4Fixture({ width: 768, height: 1152, frames: 124, includeAudio: false });
const audioMp4Bytes = buildH264AacMp4Fixture({ width: 768, height: 1152, frames: 73 });
...
function capabilityResponse(node) {
  ...
  if (node === 'KSamplerSelect') return standardInfo(node, 'sampler_name', ['res_multistep', 'euler']);
    ...
    try {
      ...
      if (url.pathname.startsWith('/history/')) {
        ...
        const filename = generatedFlf
          ...
          : loopFlf
            ? 'portrait-motion-loop-flf_00001_.mp4'
            : 'portrait-motion_00001_.mp4';
        responseJson(response, 200, {
          [historyId]: {
            status: { completed: true, status_str: 'success' },
            outputs: {
      ...
      if (url.pathname === '/view') {
        ...
        const videoBytes = state.mode === 'audio-output'
          ? audioMp4Bytes
...
test('compiled portrait-video route enforces the fake-Comfy contract', { timeout: 120_000 }, async (context) => {
  ...
  await context.test('GET returns exact MiniMax H3 capabilities through the compiled route', async () => {
    fake.reset();
    ...
    assert.deepEqual(
      ...
      new Set([...MINIMAX_H3_PORTRAIT_VIDEO_TEMPLATE.requiredNodes, ...FLUX2_KLEIN_9B_PORTRAIT_END_FRAME_TEMPLATE.requiredNodes])
    );
    assert.equal(
      queriedNodes.length,
      MINIMAX_H3_PORTRAIT_VIDEO_TEMPLATE.requiredNodes.length + FLUX2_KLEIN_9B_PORTRAIT_END_FRAME_TEMPLATE.requiredNodes.length
    );
  });

  await context.test('POST uploads exact portrait bytes and proxies the fixed H.264 video-only MP4 with provenance headers', async () => {
    fake.reset();
    ...
    assert.equal(response.headers.get('x-mullet-width'), '768');
    assert.equal(response.headers.get('x-mullet-height'), '1152');
    ...
    assert.equal(response.headers.get('x-mullet-encoded-duration-seconds'), String(73 / 24));
    assert.equal(response.headers.get('x-mullet-audio-tracks'), '0');
    assert.equal(response.headers.get('x-mullet-model-template'), 'minimax-h3-fl2va-portrait-v1');
    ...
    assert.equal(queued.prompt['6'].inputs.width, 768);
    assert.equal(queued.prompt['6'].inputs.height, 1152);
  ...
  await context.test('POST generates a FLUX.2 Klein end frame before queuing distinct-frame FLF motion', async () => {
    ...
    assert.equal(endFramePrompt.prompt['5'].inputs.megapixels, 0.9);
    assert.deepEqual(endFramePrompt.prompt['11'].inputs, { width: 768, height: 1152, batch_size: 1 });
    ...
    assert.deepEqual(videoPrompt.prompt['6'].inputs.first_frame, ['5', 0]);
  ...
  await context.test('POST rejects mismatched hash, IHDR, and multipart shape before ComfyUI', async () => {
    ...
    fake.reset();
    const wrongDimensions = png(768, 1024);
  ...
  await context.test('audio-bearing Comfy output maps to 502', async () => {
    fake.reset('audio-output');
    const response = await post(formFor(request, imageBytes));
    assert.equal(response.status, 502, await response.text());
  });
